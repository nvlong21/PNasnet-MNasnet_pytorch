...
class PNASNet5M(nn.Module):
    def __init__(self, num_classes=1001):
        super(PNASNet5M, self).__init__()
        self.num_classes = num_classes
        self.conv_0 = nn.Sequential(OrderedDict([
            ('conv', nn.Conv2d(3, 32, kernel_size=3, stride=2, bias=False)),
            ('bn', nn.BatchNorm2d(32, eps=0.001))
        ]))
        self.cell_stem_0 = CellStem0(in_channels_left=32, out_channels_left=13,
                                     in_channels_right=32, out_channels_right=13)

        self.cell_stem_1 = Cell(in_channels_left=32, out_channels_left=26,
                                in_channels_right=65, out_channels_right=26,
                                match_prev_layer_dimensions=True,
                                is_reduction=True)
        self.cell_0 = Cell(in_channels_left=65, out_channels_left=54,
                           in_channels_right=130, out_channels_right=54,
                           match_prev_layer_dimensions=True)
        self.cell_1 = Cell(in_channels_left=130, out_channels_left=54,
        ...
        self.cell_8 = Cell(in_channels_left=1080, out_channels_left=216,
                            in_channels_right=1080, out_channels_right=216)
        self.relu = nn.ReLU()
        self.avg_pool = nn.AvgPool2d((4,4), stride=1, padding=0)
        self.dropout = nn.Dropout(0.5)
        self.last_linear = nn.Linear(1080, 256)
